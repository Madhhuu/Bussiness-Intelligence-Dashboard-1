...
const EditSaleModal = ({ isOpen, onClose, onSave, saleData }) => {
    ...
    useEffect(() => {
        if (saleData) {
            const formattedDate = saleData.sale_date ? new Date(saleData.sale_date).toISOString().split('T')[0] : '';
            setFormData({
                ...saleData,
        ...
        <div style={{ position: 'fixed', inset: 0, backgroundColor: isDarkMode ? 'rgba(0, 0, 0, 0.6)' : 'rgba(15, 23, 42, 0.4)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, fontFamily: "'Inter', sans-serif" }}>
            <div style={{ backgroundColor: isDarkMode ? '#1e293b' : 'white', borderRadius: '16px', padding: '28px', width: '100%', maxWidth: '450px', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25)', border: isDarkMode ? '1px solid #334155' : 'none' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                    <h3 style={{ margin: 0, fontSize: '20px', fontWeight: '700', color: isDarkMode ? '#f8fafc' : '#0f172a' }}>Edit Sale</h3>
                ...
                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                    ...
                    <div>
                        ...
                        <input name="product_name" required value={formData.product_name} onChange={handleChange} style={inputStyle} />
                    </div>
                    <div>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px', marginTop: '12px' }}>
                        ...
                        <button type="submit" style={{ padding: '11px 18px', borderRadius: '10px', border: 'none', backgroundColor: accentColor, color: 'white', fontSize: '14px', fontWeight: '600', cursor: 'pointer', boxShadow: `0 4px 6px -1px ${accentColor}33` }}>Save Changes</button>
                    </div>
                </form>
            </div>
        </div>
